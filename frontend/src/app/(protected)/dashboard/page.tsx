...
import useGetOwnedProjects from "@/hooks/queries/projects/useGetOwnedProjects";
import { useRouter } from "next/navigation";
import useGetSharedProjects from "@/hooks/queries/projects/useGetSharedProjects";
...
export default function DashboardPage() {
  const {
    ...
    isError: isErrorOwnedProjects,
  } = useGetOwnedProjects();
  const {
    data: sharedData,
    isLoading: isLoadingSharedProjects,
    isError: isErrorSharedProjects,
  } = useGetSharedProjects();
  
  const { projects: ownedProjects } = ownedData || {};
  const { projects: sharedProjects } = sharedData || {};
  ...
  if (isLoadingOwnedProjects || isLoadingSharedProjects) {
    return <div>Please wait....</div>;
  }

  if (isErrorOwnedProjects || isErrorSharedProjects) {
      ...
      <div className="flex items-center justify-between">
        ...
        {ownedProjects?.length > 0 && (
          <div className="flex items-center gap-2 text-sm text-muted-foreground bg-gradient-to-r from-card/80 to-card/60 px-4 py-2 rounded-full backdrop-blur-sm border border-border/50">
            ...
            <span>{ownedProjects?.length} active</span>
          </div>
        )}
      </div>
            ...
            >
              ...
              <CardContent className="relative space-y-6">
                ...
                <Button
                  ...
                  className="w-full h-12 text-white font-medium transition-all duration-300 hover:scale-105"
                >
